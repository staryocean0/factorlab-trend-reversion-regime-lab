...
- fixed-v5 causal decoded-state age in bars, capped at 20;
- the existing 12 frozen causal K-line features from v4.

The state-age feature is generated by replaying the already-frozen v5 causal decoder on the same state-head probabilities. The same v5 replay definition is used in fitting and online scoring, so no judge state or future label is used to construct state age.
...
3. replay the frozen v5 decoder in parallel solely to provide its causal state-age feature;
