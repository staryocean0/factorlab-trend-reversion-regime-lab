...
>
> 本文在任何五桶 forward outcome、转移概率、反转率或 excursion 结果被计算之前冻结。机器可读合同：[`TREND_FIVE_BUCKET_PROTOCOL_M4_V1.json`](TREND_FIVE_BUCKET_PROTOCOL_M4_V1.json)。
...
如果 `T2=4` 样本不足，结论是 `INCONCLUSIVE`，不是降低阈值重新试到显著为止。
