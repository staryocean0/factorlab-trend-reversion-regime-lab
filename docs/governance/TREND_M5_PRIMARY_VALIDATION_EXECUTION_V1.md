...
同一次运行还输出 M4 预注册 descriptive metrics；split 末端仍未自然结束的 duration episode 按右删失单列，不进入完成 episode 的均值/中位数。M5-4 完成后只允许进入 M5-5 预注册 `T2=3/5` Validation sensitivity；Holdout 仍锁定。

Final execution trigger: armed once after all code/config/data identities and split-safe descriptive rules were locked.
