...
PROTOCOL_PATH = ROOT / "docs/research/KLINE_RECOGNIZER_SHOCK_EXIT_INERTIA_V12_PROTOCOL.md"
...
REQUIRED_CONTRIBUTION = "v6_soft_transition_asymmetry"
AUX_C = 0.1
...
def _result_card(selected, selected_aggregate, best, best_aggregate, champion_aggregate, challenger_2025, champion_2025, veto):
    lines = [
        "# K-line recognizer Shock-exit soft-inertia v12 — result card",
        "",
        "V12 integrates exactly one audited historical contribution into champion v10: a soft Shock-exit inertia derived from v6. All v10 heads, alpha and ordinary decoder behavior remain fixed.",
        "",
        "## Champion before v12",
        "",
        ...
        "## Pre-2025 decision",
        "",
        f"V12 promotion candidate found: **{selected is not None}**",
        "",
    ]
    if best is not None:
        lines.extend([
            "Best local v12 candidate:",
    ...
    if selected is None:
        lines.extend([
            "No v12 gamma passed the frozen v10 noninferiority + material-improvement gate. V10 remains champion.",
            "",
            "Champion update eligibility: **False**",
            "",
            "Any useful local Shock-exit effect may still be retained in the Contribution Ledger.",
        ])
    else:
        lines.extend([
            f"Selected gamma: `{selected.gamma:.2f}`",
        ...
        for symbol in ("000852.SH", "000688.SH"):
            for label, source in (("v12", challenger_2025), ("v10 champion", champion_2025)):
                m = source[symbol]
                lines.append(
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run frozen v12 Shock-exit soft-inertia integration.")
    parser.add_argument("--code-commit")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "experiments/kline_recognizer_shock_exit_inertia_v12",
    )
...
def main() -> int:
    ...
    for path in (PROTOCOL_PATH, CHAMPION_PATH, CONTRIBUTION_PATH):
        if not path.is_file():
            raise RuntimeError(f"v12 required authority input missing: {path}")
    ...
    if registry.get("champion_version") != "v10_temporal_blend":
        raise RuntimeError("v12 is frozen against v10 champion")
    if registry.get("champion_result_commit") != EXPECTED_CHAMPION_COMMIT:
        raise RuntimeError("v12 champion result commit mismatch")
    champion_aggregate = registry["pre2025_worst_cell"]

    ledger = json.loads(CONTRIBUTION_PATH.read_text(encoding="utf-8"))
    ids = {row.get("contribution_id") for row in ledger.get("contributions", [])}
    if REQUIRED_CONTRIBUTION not in ids:
        raise RuntimeError("v12 required audited v6 contribution is missing")
    ...
    for fold_name, train_end, _, _ in FOLDS:
        ...
        if primary is None:
            raise RuntimeError("v12 primary model unexpectedly absent")
    ...
    rows = []
    selection_rows = []
    for candidate in frozen_shock_exit_menu():
        fold_results = {}
        for fold_name, _, val_start, val_end in FOLDS:
            bundle = fold_models[fold_name]
            results, _, _ = score_shock_exit_period(
                frames,
                contexts,
                judge_events,
                bundle["primary"],
    ...
    selected, selected_aggregate, best, best_aggregate = select_shock_exit_candidate(
    ...
    if selected is not None:
        ...
        if primary is None:
            raise RuntimeError("v12 refit primary absent")
        temporal = fit_temporal_linear(
            frames,
            contexts,
            TemporalContextCandidate(6, AUX_C),
        ...
        challenger_2025, _, _ = score_shock_exit_period(
        ...
        champion_2025, _, _ = score_blend_period(
            frames, contexts, judge_events, primary, temporal, V10_BLEND,
            start=DIAGNOSTIC_START, end=DIAGNOSTIC_END,
        )
        veto = safety_veto_2025(challenger_2025, champion_2025)
        selected_model = {
            "candidate": selected.to_dict(),
            "fixed_v10_blend": V10_BLEND.to_dict(),
    ...
    summary = {
        "status": "complete_shock_exit_inertia_v12",
        "champion_before_run": registry,
        "source_contribution": REQUIRED_CONTRIBUTION,
    ...
    if _sha256(CHAMPION_PATH) != champion_hash:
        raise RuntimeError("v12 mutated champion registry")
    if _sha256(CONTRIBUTION_PATH) != contribution_hash:
        raise RuntimeError("v12 mutated contribution ledger")
