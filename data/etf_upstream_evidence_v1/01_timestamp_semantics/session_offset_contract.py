...
It is not Hilbert/IIR filter phase, not the 3s transaction
``session_phase`` label, and not FactorLab CloudRidge continuous clock.

DataHub constructs official and wall-clock bars from 1m raw. FactorLab
owns the research menu, fill pairing and inventory gates. Continuous
noon-crossing bars stay out of this contract.
...
BAR_ALIGN_CONTINUOUS = "session_clock_continuous"
SUPPORTED_BAR_ALIGNS = (BAR_ALIGN_OFFICIAL, BAR_ALIGN_WALL_CLOCK)
FORBIDDEN_BAR_ALIGNS = (BAR_ALIGN_CONTINUOUS,)
...
# Capability gold samples only. FactorLab research defaults are 60m+30 / 15m+5.
DEFAULT_RESEARCH_OFFSETS = GOLD_SAMPLE_OFFSETS
...
def normalize_session_offset_request(
    *,
    ...
    if align in FORBIDDEN_BAR_ALIGNS:
        raise ValueError(
            f"unsupported bar_align={bar_align!r}; "
            "session_clock_continuous is a FactorLab CloudRidge research "
            "carrier and is not a DataHub wall-clock product"
        )
